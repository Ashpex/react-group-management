...
const MultipleChoiceSlideContentSetting = ({ slideInfo, form }) => {
  ...
  useEffect(() => {
    if (slideInfo?.title) {
      form.setFieldValue(
        "question",
        slideInfo?.title !== "New Slide" ? slideInfo?.title : ""
      );
    }
...
export default function EditPresentation() {
  ...
  useEffect(() => {
    if (presentationData) {
      const currentSlideData = presentationData.slides.find(
        (i) => i._id === slideId
      );
  ...
  const handleCreateNewSlide = async () => {
    try {
      const { data: response } = await presentationApi.createSlide(
        presentationId
      );
  ...
  const handleSave = async () => {
    try {
      const { data: response } =
        await presentationApi.updateMultipleChoiceSlide(slideId, {
          ...
          {isLoading ? (
            ...
            <Grid>
              ...
              <Grid.Col span={4} p={16}>
                ...
                />
                <Divider my="md" />
                <MultipleChoiceSlideContentSetting
                  slideInfo={slideData}
                  form={form}
                />
                <Divider my="xl" />
